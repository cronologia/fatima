# AGENTS.md — cronologia/fatima

Operating guide for AI coding agents (and humans) working in this repository.

**Before you touch `data/*.json`, load two skills and follow them:**

- **`sourcing-rules`** (`cronologia/core`) — the five sourcing rules that govern
  every repo in the organization. This subject makes rule 2 (*attribute, don't
  assert*) load-bearing on almost every field: see "Data quality & sourcing
  rules" below.
- **`data-edit`** — the mandatory data-edit gate: query → edit →
  `node scripts/validate-data.js` → `node build.js` → `node --test` → commit the
  data change **and** the regenerated `docs/` in one commit. Never hand-edit
  anything under `docs/`.

Read `context.md` (current state, open questions) and `KEYWORDS.md` (name
variants and known search traps) before researching or searching for sources.
The shared method lives in `cronologia/core` (skills: sourcing-rules,
bootstrap-project, data-edit, release-work, preserve-sources, mine-video,
...
apparitions at Fátima, Portugal (1916–2017)** — what the three seers reported
and when, what the contemporary press recorded, what the Church ruled and in
which document, and the later beatifications, canonizations and the 2000
publication of the "Third Secret".
...
data/i18n/{es,pt}.json   Translation caches (committed). In THIS repo they are HAND-AUTHORED, not machine-translated — see _meta.generatedBy. Edit them by hand when the English prose changes; do not overwrite them with scripts/translate.js output without saying so in _meta.
...
The five rules of the `sourcing-rules` skill apply verbatim. This subject makes
four of them concrete — treat these as non-negotiable house style:

1. **An apparition is a REPORTED event, never an asserted one.** Every event
   entry says *who reported what, and when*. Write "the children reported…",
   "the press reported…", never "the Virgin appeared". The site's own voice
   never adjudicates the supernatural claim, in either direction — it neither
   asserts nor debunks.
2. **A Church judgment is an act with a document and a date.** The key one is
   the pastoral letter **"A Divina Providência" of 13 October 1930**, in which
   Bishop **José Alves Correia da Silva** of Leiria declared the visions
   *dignas de crédito* ("worthy of belief") and officially permitted the cult.
   Always cite the document (`fatima-dcf-selecao`, doc. 133), always say it is
   a **diocesan** judgment of credibility permitting devotion — a private
   revelation that obliges no one to believe, not a dogmatic definition. There
   is no separate "papal approval" decree; papal acts (visits, the 2000
   beatification, the 2017 canonization) embraced the diocesan approval.
3. **The "Miracle of the Sun" is attributed to the contemporary press, never
   asserted.** The attestation is **Avelino de Almeida** in **O Século**,
   15 October 1917 ("Coisas espantosas! Como o sol bailou ao meio-dia em
   Fátima"), plus Judah Ruah's crowd photographs in **Ilustração Portuguesa**,
   29 October 1917. O Século was an anticlerical republican paper; Almeida
   reported what the crowd said it saw and framed "miracle" as the crowd's cry.
   Citing him attests the event's contemporaneity and scale — nothing more.
4. **1917 detail is mostly attested retrospectively, and that must stay
   visible.** The 1916 Angel apparitions, the vision of hell and the three-part
   "Secret" come from **Lúcia's memoirs (1935, 1937, 1941 ×2)** and her
   manuscript of **3 January 1944**, i.e. 18–27 years after the events; the
   third part was published only on **26 June 2000** by the CDF. Any new event
   drawing on the memoirs repeats that caveat in its own `text`. It is a
   sourcing caveat, not a verdict on the texts' truthfulness.
5. **Perspective labels are required.** The Sanctuary and vatican.va are
   official and sympathetic custodians; Agência ECCLESIA is the Portuguese
   Bishops' Conference's agency; Observador is independent press. Every
   reference says so in `publisherNote` (which IS translated) — never glued
   into `publisher` as a parenthetical (a test enforces this).
